...
{{--@section('page_header')--}}
    {{--<h1 class="page-title">--}}
        ...
        {{--@if (!is_null($dataTypeContent->getKey()))--}}
            {{--{{ __('voyager::generic.edit') }} {{ $dataTypeContent->name }}--}}
        {{--@else--}}
...
@section('content')
    <div class="page-content edit-add container-fluid" id="edit-add">
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="panel panel-bordered">
                    ...
                    <form role="form"
                            class="form-edit-add"
                            action="@if(!is_null($dataTypeContent->getKey())){{ route('voyager.'.$dataType->slug.'.update', ['vid' => $vid, 'id' => $dataTypeContent->getKey()]) }}@else{{ route('voyager.'.$dataType->slug.'.store', ['vid' => $vid]) }}@endif"
                            method="POST"
                        ...
                        <div class="panel-body">
                            ...
                            @foreach($dataTypeRows as $row)
                                ...
                                @php
                                    ...
                                    if ($row->field == 'vid') {
                                        $row->details = ['default' => $vid];
                                    }

                                    if ($row->field == 'parent_id') {
                                        $details = (array)$row->details;
                                        $details['default'] = request()->input('parent_id');
                                        $row->details = $details;
                                    }
                                    if (isset($row->details->onlyVid) && $vid != $row->details->onlyVid) {
                                        continue;
                                    }
...
@section('javascript')
    <script>
        ...
        $('document').ready(function () {
            ...
            $(".form-edit-add").ajaxForm(function(res) {
                if (currentNode)
                    currentNode.isParent = true;
                zTreeObj.reAsyncChildNodes(currentNode, 'refresh');
                if (currentNode && res.data.parent_id != currentNode.id) {
                    var node = zTreeObj.getNodeByParam("id", res.data.parent_id);
                    if (node != null)
                        zTreeObj.reAsyncChildNodes(node, 'refresh');
                }
                // Reload #edit-term elem
                $("#edit-term").load($("#edit-term").attr('p-href'))
            })
        });
    </script>
@stop
